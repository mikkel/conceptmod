"""SMOKE (not CI): Qwen-Image baseline generate + one velocity-space op step.

Requires CUDA and a download of ``Qwen/Qwen-Image`` (~20B DiT). This is not
a substitute for a full train — it only checks that the backend loads, a
LoRA attaches, and one DSL step produces a gradient.

    CONCEPTMOD_DEVICE=cuda:0 python scripts/smoke_qwen.py

Skip this on CPU / small VMs. Mapper + convert coverage lives in
``tests/test_convert_lora_comfyui.py`` (no weights).
"""
import os
import time
...
MODEL = os.environ.get("QWEN_MODEL", "Qwen/Qwen-Image")

if not torch.cuda.is_available():
    raise SystemExit(
        "smoke_qwen.py is a labeled GPU smoke path; CUDA is required. "
        "Run the convert/mapper unit tests instead."
    )

t0 = time.time()
backend = load_backend("qwen", device=DEVICE, lora_rank=16, model_id=MODEL,
                       resolution=512)
...
os.makedirs("outputs/50_qwen_baseline", exist_ok=True)
prompts = [
...
for i, p in enumerate(prompts):
    ...
    slug = p.replace(" ", "_")[:36]
    img.save(f"outputs/50_qwen_baseline/{i}_{slug}.png")
