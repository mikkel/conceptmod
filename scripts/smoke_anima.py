"""Smoke test Anima backend: baseline generation + one op step."""
import os
import time
...
t0 = time.time()
backend = load_backend("anima", device=DEVICE, lora_rank=16)
...
os.makedirs("outputs/30_anima_baseline", exist_ok=True)
prompts = [
    "masterpiece, best quality, safe, score_7, 1girl, solo, school uniform, "
    "standing, city lights, looking at viewer, outdoors, full body",
    "masterpiece, best quality, safe, score_7, a calico cat sitting on a "
    "windowsill, anime, indoors, daylight",
    "masterpiece, best quality, safe, score_7, a bowl of fruit on a wooden "
    "table, anime still life",
]
for i, p in enumerate(prompts):
    t1 = time.time()
    img = backend.generate(p, seed=42 + i)
    slug = p.replace(" ", "_")[:36]
    img.save(f"outputs/30_anima_baseline/{i}_{slug}.png")
...
cfg = ops.OpDefaults(sample_steps=14, sample_guidance=4.5)
